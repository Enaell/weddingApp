var projectsContainer = $('.projects-wrapper');
var projectsSlider = projectsContainer.children('.slider');
...
jQuery(document).ready(function ()
{
	...
	function setSliderContainer() {
		...
		if(mq == 'desktop') {
			...
			var marginLeft = Number(projectsSlider.children('li').eq(1).css('margin-left').replace('px', ''));
			...
			projectsSlider.css('height', 1.4 * slideWidth);
			$('#morePict').css('padding-top', slideWidth * 0.6);
			( slideCurrentIndex != 0 ) && setTranslateValue(projectsSlider, (  slideCurrentIndex * (slideWidth + marginLeft) + 'px'));
		} else {
			projectsSlider.css('width', '');
			projectsSlider.css('height', '');
			$('#morePict').css('padding-top', '100px');
